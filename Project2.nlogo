undirected-link-breed [redlinks redlink]
directed-link-breed [bluelinks bluelink]
breed [cells cell]
...
[
  ...
  mem1
  CM
  last-cell ;; keeps track of whether last node has fired
]
muscle-cells-own [
  old-voltage  ;; the voltage of the patch the last time thru go
  voltage  ;; the current voltage of the patch
...
to setup
  ...
  set internode 4
  ;let rho .07
  set fiberdiam 0.001; ;; 0.001 = 10 um
  set dt 0.0001
  ...
  create-cells 100 [set shape "square"  set size 4 set last-cell 0]
  let kkk 0
  while [ kkk < 100  ] [
  let llc-x kkk * 4 + 2 + 2
  let llc-y 2
  ...
  ;let j-m 0
  ;while [ i-m < 100 ] [
   ; ask muscle-cell i-m [ setxy j-m + 2 5 ]
    ;set i-m i-m + 1
    ;set j-m j-m + 4
  ;]
  ; create muscle cell links
  let k-m 100
  while [ k-m < 200] [
    ask muscle-cell k-m [ create-redlink-with muscle-cell (k-m + 1) ]
    set k-m k-m + 1
  ]
  ask muscle-cells
  [
    ...
    set nold n
    set hold h
    set conductivity radius / (2 * dxx * dxx * rho)
    set color gray
  ]

  ask cell 99 [ create-bluelink-to muscle-cell 150 [set thickness 3 set color blue]]

  ;;place rec electrode
  ask patch (rec1 * 4 + 2) 9 [set plabel "*" set plabel-color black]
  ask patch (rec2 * 4 + 2) 9 [set plabel "*" set plabel-color black]
  ask patch 420 ((rec3 - 100) * 4 - 198) [set plabel "*" set plabel-color black]
  ask patch 420 ((rec4 - 100) * 4 - 198) [set plabel "*" set plabel-color black]
  ...
  let k 0
  while [ k < 99 ] [
  ask cell k [ create-redlink-with cell (k + 1) ]
    set k k + 1
  ]
...
to go
  ...
  ask cells [
    ...
    ifelse who = 99 [
      set voltage old-voltage + (dt / CM) *  ( (conductance / (2 * pi * radius * mem1))  * ([old-voltage] of cell 98) -  ( conductance / (2 * pi * radius * mem1) ) * old-voltage
        -(INa + IK + IL + IM) + istim)
      ifelse old-voltage < 0 and voltage > 0 [
        ; 1 is a value > V-threshold
        ask cell 99 [set last-cell 1]
       ; ask muscle-cell 150 [set last-cell 1]
      ] [ ask cell 99 [set last-cell 0]]
    ]
  ...
  ask muscle-cells [
    ask muscle-cell 150 [
      let am (0.1 * (25.0 - (old-voltage + 60.) )) / (e ^ (2.5 - 0.1 * ((old-voltage + 60.0) )) - 1)
      let bm (4 * e ^ ((- (old-voltage + 60.0))/ 18))
      let ah (0.07 * e ^ ( (- (old-voltage + 60.0) )/ 20))
      let bh (1 / (e ^ (3 - 0.1 * ((old-voltage + 60.0))) + 1))
      let an ((0.1 - 0.01 * ((old-voltage + 60.0))) / (e ^ (1 - 0.1 * ((old-voltage + 60.0))) - 1))
      let bn (0.125 * e ^ ((- (old-voltage + 60.0) )/ 80) )
      set m mold + dt * ( am * (1 - mold) - bm * mold)
      set n nold + dt * ( an * (1 - nold) - bn * nold)
      set h hold + dt * ( ah * (1 - hold) - bh * hold)
      let INa GNa * mold ^ 3 * hold * (old-voltage - ENa)
      ...
      let IL  GL * (old-voltage - EL)
      let IM  Gm * (old-voltage)
      set gsyn gsyn + (- gsyn / 3.0 + sum [last-cell] of cells / dt) * dt
      ;ask cell 99 [set last-cell 0]
      ;set last-cell 0
      let IS weight * (gsyn * (old-voltage - 10))
      ;let isynapse
      set voltage old-voltage +  dt * ( (conductivity * (sum [old-voltage] of link-neighbors)) - ( count link-neighbors * conductivity ) * old-voltage  -(INa + IK + IL + IS) )
      ;set voltage old-voltage +  dt * IS
      set mold m
      set nold n
      set hold h
    ]
    ...
    let IM  Gm * (old-voltage)
    ;let isynapse
    set voltage old-voltage +  dt * ( (conductivity * (sum [old-voltage] of link-neighbors)) - ( count link-neighbors * conductivity ) * old-voltage  -(INa + IK + IL ) )
    set old-voltage voltage
    set mold m
    set nold n
    set hold h
  ...
  set outvol3 [voltage] of muscle-cell rec3
  set outvol4 [voltage] of muscle-cell rec4
  set gsynapse [gsyn] of muscle-cell 150
  set showlc [last-cell] of cell 99
...
to color-turtle  ;; Patch Procedure
  ask cells [set color scale-color red voltage -65 50 ]
  ask muscle-cells [set color scale-color red voltage -65 50 ]
end
to do-plot
  ...
  if plot-pen-exists? "outvol3" [
    set-current-plot-pen "outvol3"
    plotxy ticks outvol3
  ]
  if plot-pen-exists? "outvol4" [
    set-current-plot-pen "outvol4"
    plotxy ticks outvol4
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"outvol3" 1.0 0 -16777216 true "" "plot outvol3"
"outvol4" 1.0 0 -7500403 true "" "plot outvol4"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot gsynapse"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot showlc"
